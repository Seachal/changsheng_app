...
class CSClassStringUtils{


  static String csMethodNumberConversion(String value,{int asFixed:-1}){
    var result=0.0;
    if(value==null){
  ...
  static String csMethodMaxLength(String string,{ int ?length}){
  ...
  static String csMethodSqlitZero(String string){
    if(string==null||string.isEmpty){
      return "";
    }
   ...
   var value=  int.tryParse(string);
    ...
    while((string.endsWith("0")||string.endsWith("."))&&string.length>1){
      bool csProCanReturn=string.endsWith(".");
  ...
  static String csMethodMatchStatusString(bool csProIsOver,String decStatus,csProStTime,{String ?status}){
    ...
    if(decStatus!=null&&decStatus.trim().isNotEmpty){
      ...
      return decStatus;
        ...
        if(  DateTime.parse(csProStTime).difference(DateTime.now()).inSeconds>0){
          return "未";
        }else{
          return "进行中";
        }
  ...
  static bool csMethodIsNum(String value){
    if(value==null){
      return false;
    }
  ...
  static String csMethodPanKouData(String value,{String showText:""}){
  ...
  static String csMethodNumK(String value)
  {
      if(!csMethodIsNum(value)){
        return "";
      }
    var csProMoney=  double.parse(value);
      if(csProMoney>1000){
        return (csProMoney/1000).toStringAsFixed(1)+"K";
      }

    return csProMoney.toStringAsFixed(0);
  ...
  static csMethodIsEmpty(String value){
    if(value==null||value.isEmpty){
      return true;
    }
    return false;
